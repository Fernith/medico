...
interface RutinaFormProps { initialData?: Rutina | null; onSuccess: () => void; onCancel: () => void; }

const FaseBuilder: React.FC<{ fase: string; items: ItemFase[]; realizaciones: RealizacionEjercicio[]; onAdd: (item: ItemFase) => void; onRemove: (tempId: string) => void; }> = ({ fase, items, realizaciones, onAdd, onRemove }) => {
  ...
  const handleAdd = () => {
    ...
    if (!real) return;
    onAdd({ tempId: Math.random().toString(36).substr(2, 9), realizacion_id: selId, ejercicio_nombre: real.ejercicio_nombre, descanso_posterior: descanso });
    setSelId(null); setDescanso('');
  };
      ...
      <div className="p-4 space-y-4">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1">
            <Select 
              searchable placeholder="Selecciona un ejercicio planificado..." value={selId} onChange={setSelId}
              // FILTRAMOS PARA QUE SOLO SE PUEDAN AÑADIR ACTIVOS
              options={realizaciones.filter(r => r.activo !== false).map(r => ({ 
                value: r.id, 
                label: `${r.ejercicio_nombre} - ${r.series ? r.series+'x' : ''}${r.reps_min || '?'}`
              }))}
...
export const RutinaForm: React.FC<RutinaFormProps> = ({ initialData, onSuccess, onCancel }) => {
  ...
  const [formData, setFormData] = useState({ nombre: initialData?.nombre || '', descripcion: initialData?.descripcion || '', color: initialData?.color || '#4f46e5' });
  const [fases, setFases] = useState<Record<string, ItemFase[]>>({ Calentamiento: [], Principal: [], Postentreno: [] });
    ...
    try {
      ...
      for (const [faseNombre, items] of Object.entries(fases)) {
        items.forEach((item, index) => {
          inserciones.push({ rutina_id: rutinaId!, realizacion_id: item.realizacion_id, fase: faseNombre, orden: index + 1, descanso_posterior: item.descanso_posterior ? parseInt(item.descanso_posterior) : null });
        });
      }
  ...
  const handleAddFase = (fase: string, item: ItemFase) => setFases(prev => ({ ...prev, [fase]: [...prev[fase], item] }));
  const handleRemoveFase = (fase: string, tempId: string) => setFases(prev => ({ ...prev, [fase]: prev[fase].filter(i => i.tempId !== tempId) }));

  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-8 pr-2 custom-scrollbar">
      <div className="bg-indigo-50/50 p-5 rounded-2xl border border-indigo-100 space-y-4">
        <h3 className="font-bold text-indigo-900 text-lg flex items-center gap-2 mb-2"><AlignLeft className="w-5 h-5 text-indigo-500"/> Datos Generales</h3>
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-start">
          ...
          <div className="space-y-1">
            <label className="block text-sm font-bold text-slate-700 ml-1">Color</label>
      ...
      <div className="space-y-6">
        ...
        {['Calentamiento', 'Principal', 'Postentreno'].map(fase => (
          <FaseBuilder key={fase} fase={fase} items={fases[fase]} realizaciones={realizaciones} onAdd={(item) => handleAddFase(fase, item)} onRemove={(tempId) => handleRemoveFase(fase, tempId)} />
        ))}
      </div>
