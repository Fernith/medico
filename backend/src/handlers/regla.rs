...
pub async fn get_ciclos(State(pool): State<PgPool>) -> Result<Json<Vec<CicloMenstrual>>, AppError> {
...
pub async fn get_ciclo_actual(
    State(pool): State<PgPool>,
) -> Result<Json<CicloMenstrual>, AppError> {
    ...
    match ciclo {
        Some(c) => Ok(Json(c)),
        None => Err(AppError::NotFound("Registro no encontrado".to_string())), // 404 significa "No hay ciclo activo", el frontend lo manejarÃ¡.
    }
}
...
pub async fn create_ciclo(
...
) -> Result<(StatusCode, Json<CicloMenstrual>), AppError> {
    ...
    .bind(payload.estado_animo)
    .bind(payload.sensacion)
    .fetch_one(&pool)
    .await?;
