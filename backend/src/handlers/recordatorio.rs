...
pub async fn get_recordatorios(
    State(pool): State<PgPool>,
) -> Result<Json<Vec<Recordatorio>>, AppError> {
    let recs = sqlx::query!(
        "SELECT clave, nombre, descripcion, dias, entidad, proxima_fecha FROM recordatorios"
    )
    .fetch_all(&pool)
    .await?;
    ...
    for r in recs {
        ...
        if r.entidad == "fecha" {
            ...
            match r.entidad.as_str() {
                "peso" => {
                    ultima_fecha = sqlx::query!("SELECT MAX(fecha) as f FROM pesos")
                        .fetch_one(&pool)
                        .await
                        .ok()
                        .and_then(|row| row.f);
                ...
                "medicion" => {
                    ultima_fecha = sqlx::query!("SELECT MAX(fecha) as f FROM medicion")
                        .fetch_one(&pool)
                        .await
                        .ok()
                        .and_then(|row| row.f);
                ...
                "pasos" => {
                    ultima_fecha = sqlx::query!("SELECT MAX(fecha) as f FROM pasos")
                        .fetch_one(&pool)
                        .await
                        .ok()
                        .and_then(|row| row.f);
                ...
                "sueno" => {
                    ultima_fecha = sqlx::query!("SELECT MAX(fecha) as f FROM sueno")
                        .fetch_one(&pool)
                        .await
                        .ok()
                        .and_then(|row| row.f);
                ...
                "entrenamiento" => {
                    ultima_fecha =
                        sqlx::query!("SELECT MAX(DATE(fecha_fin)) as f FROM historial_rutinas")
                            .fetch_one(&pool)
                            .await
                            .ok()
                            .and_then(|row| row.f);
        ...
        resultados.push(Recordatorio {
            ...
            alerta: Some(alerta),
            dias_extra: Some(dias_extra),
        });
    }
    Ok(Json(resultados))
}

pub async fn create_recordatorio(
    State(pool): State<PgPool>,
    Json(payload): Json<Recordatorio>,
...
pub async fn update_recordatorio(
...
pub async fn delete_recordatorio(
    Path(clave): Path<String>,
    State(pool): State<PgPool>,
) -> Result<Json<()>, AppError> {
    sqlx::query!("DELETE FROM recordatorios WHERE clave = $1", clave)
        .execute(&pool)
        .await?;
    Ok(Json(()))
}
...
pub async fn posponer_recordatorio(
    Path(clave): Path<String>,
    State(pool): State<PgPool>,
) -> Result<Json<()>, AppError> {
    let hoy = Local::now().naive_local().date();
    let rec = sqlx::query!(
        "SELECT dias FROM recordatorios WHERE clave = $1",
        clave.clone()
    )
    .fetch_one(&pool)
    .await?;
    ...
    sqlx::query!(
        "UPDATE recordatorios SET proxima_fecha = $1 WHERE clave = $2",
        nueva_fecha,
        clave
    )
    .execute(&pool)
