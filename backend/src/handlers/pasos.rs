...
use crate::models::pasos::{PasoDB, PasoPayload, PasosDashboardResponse};
use axum::{
    extract::{Path, State},
    Json,
};
...
) -> Result<Json<PasosDashboardResponse>, AppError> {
...
// ==========================================
pub async fn get_historial(State(pool): State<PgPool>) -> Result<Json<Vec<PasoDB>>, AppError> {
    let pasos = sqlx::query_as!(
        PasoDB,
        "SELECT id, fecha, cantidad, creado_en FROM pasos ORDER BY fecha DESC"
    )
    .fetch_all(&pool)
    .await?;
    Ok(Json(pasos))
...
pub async fn create_paso(
    State(pool): State<PgPool>,
    Json(payload): Json<PasoPayload>,
...
pub async fn update_paso(
    Path(id): Path<Uuid>,
    State(pool): State<PgPool>,
    Json(payload): Json<PasoPayload>,
) -> Result<Json<()>, AppError> {
    sqlx::query!(
        "UPDATE pasos SET fecha=$1, cantidad=$2 WHERE id=$3",
        payload.fecha,
        payload.cantidad,
        id
    )
...
pub async fn delete_paso(
    Path(id): Path<Uuid>,
    State(pool): State<PgPool>,
) -> Result<Json<()>, AppError> {
    sqlx::query!("DELETE FROM pasos WHERE id=$1", id)
        .execute(&pool)
        .await?;
    Ok(Json(()))
}
